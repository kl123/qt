...
MainWindow::MainWindow(QWidget *parent)
  ...
  std::cout << "OpenCV image processing works! Size: " << testImg.size()
            << std::endl;
  ...
  croppedLabel->setStyleSheet(
      "QLabel { background-color: #333; border: 1px solid #666; }");
  ...
  connect(continuousTimer, &QTimer::timeout, this,
          &MainWindow::onContinuousCapture);
  connect(AreaButton, &QPushButton::clicked, this,
          &MainWindow::onContinuousDetect);
...
MainWindow::~MainWindow() {
  if (timer->isActive())
    timer->stop();
  if (continuousTimer && continuousTimer->isActive())
...
// 停止监测（通用）
void MainWindow::onStopMonitoring() { stopAllMonitoring(); }
...
cv::Mat MainWindow::cropImageByPercent(const cv::Mat &src, int startXPercent,
                                       int widthPercent, int startYPercent,
                                       int heightPercent) {
  if (src.empty()) {
    return cv::Mat();
  }
...
void MainWindow::onAreaParamsReceived(int startXPercent, int widthPercent,
                                      int startYPercent, int heightPercent) {
  // 存储参数
  ...
  logTextEdit->append(
      QString("→ X起始：%1%，宽度：%2%").arg(startXPercent).arg(widthPercent));
  logTextEdit->append(
      QString("→ Y起始：%1%，高度：%2%").arg(startYPercent).arg(heightPercent));
  logTextEdit->append("----------------------------------");
  ...
  if (!isContinuousDetecting) {
    QSettings settings("MyCompany", "MonitorApp");
    int interval =
        settings.value("detectionInterval", 2).toInt() * 1000; // 默认2秒
...
void MainWindow::onCaptureScreen() {
  ...
  cv::Mat mat;
  if (image.format() == QImage::Format_RGB888) {
    mat = cv::Mat(image.height(), image.width(), CV_8UC3, (uchar *)image.bits(),
                  image.bytesPerLine());
  ...
  if (!isMonitoringActive) {
    int elapsedSeconds =
        monitoringStartTime.secsTo(QDateTime::currentDateTime());
    QSettings settings("MyCompany", "MonitorApp");
    int intervalSeconds = settings.value("detectionInterval", 5).toInt();
  ...
  try {
    ...
    if (!changedFrame.empty()) {
      ...
      cv::Mat beforeChange =
          prevFrame.empty() ? changedFrame.clone() : prevFrame;
      cv::Mat afterChange = changedFrame.clone();

      cv::Mat croppedBefore =
          m_imageProcessor.CutPhoto(beforeChange, isDarkMode);
      ...
      for (const QString &keyword : alertKeywords) {
        if (!newMessage.isEmpty() &&
            newMessage.contains(keyword, Qt::CaseInsensitive)) {
          keywordFound = true;
        ...
        if (countAfter > countBefore) {
          keywordFound = true;
          triggerReason = QString("关键词增量: %1 (%2->%3)")
                              .arg(keyword)
                              .arg(countBefore)
                              .arg(countAfter);
          break;
      ...
      QImage img1((const uchar *)displayOriginal.data, displayOriginal.cols,
                  displayOriginal.rows, displayOriginal.step,
                  QImage::Format_RGB888);
      imageLabel->setPixmap(QPixmap::fromImage(img1).scaled(
      ...
      if (!croppedAfter.empty()) {
        ...
        QImage img2((const uchar *)displayCropped.data, displayCropped.cols,
                    displayCropped.rows, displayCropped.step,
                    QImage::Format_RGB888);
        croppedLabel->setPixmap(QPixmap::fromImage(img2).scaled(
            croppedLabel->size(), Qt::KeepAspectRatio,
...
cv::Mat MainWindow::detectAndCaptureChanges(const cv::Mat &currentFrame) {
  ...
  if (changeRatio > MIN_CHANGE_RATIO) {
    std::cout << "监测到消息！变化比例: " << (changeRatio * 100) << "%"
              << std::endl;
    lastFrame = currentFrame.clone();
...
void MainWindow::onTestAlert() { m_imageProcessor.StartAlert(); }
...
void MainWindow::onContinuousCapture() {
  ...
  if (!screen) {
    logTextEdit->append(
        "<font color='orange'><b>[连续检测] 无法获取屏幕！</b></font>");
    return;
  }
  ...
  if (pixmap.isNull()) {
    logTextEdit->append(
        "<font color='orange'><b>[连续检测] 截图失败！</b></font>");
    return;
  }

  // 2. 转换为OpenCV Mat（BGR格式）
  QImage image = pixmap.toImage().convertToFormat(QImage::Format_RGB888);
  cv::Mat currentFrame(image.height(), image.width(), CV_8UC3,
                       (uchar *)image.bits(), image.bytesPerLine());
  cv::cvtColor(currentFrame, currentFrame, cv::COLOR_RGB2BGR);

  // 3. 根据接收的参数裁剪监测区域
  cv::Mat croppedFrame = cropImageByPercent(
      currentFrame, m_areaStartX, m_areaWidth, m_areaStartY, m_areaHeight);
  if (croppedFrame.empty()) {
    logTextEdit->append(
        "<font color='orange'><b>[连续检测] 裁剪监测区域失败！</b></font>");
    return;
  }
  ...
  if (!m_croppedLastFrame.empty()) {
    ...
    changeRatio = totalPixels > 0
                      ? static_cast<double>(changedPixels) / totalPixels
                      : 0.0;
  }
  ...
  const double MIN_CHANGE_RATIO = 0.003;
  if (changeRatio > MIN_CHANGE_RATIO) {
    logTextEdit->append(QString("<font color='red'><b>[连续检测] "
                                "监测区域发现变化！变化比例: %1%</b></font>")
  ...
  cv::rectangle(displayOriginal, cv::Point(x, y), cv::Point(x + w, y + h),
                cv::Scalar(0, 0, 255), 2);
  cv::cvtColor(displayOriginal, displayOriginal, cv::COLOR_BGR2RGB);
  ...
  cv::cvtColor(displayCropped, displayCropped, cv::COLOR_BGR2RGB);
  QImage imgCropped(displayCropped.data, displayCropped.cols,
                    displayCropped.rows, displayCropped.step,
                    QImage::Format_RGB888);
  croppedLabel->setPixmap(QPixmap::fromImage(imgCropped)
                              .scaled(croppedLabel->size(), Qt::KeepAspectRatio,
                                      Qt::SmoothTransformation));
}
...
void MainWindow::mode1() {
  ...
  if (!hasDetectConfig) {
    ...
    logTextEdit->append(
        ...
        "启动失败：未检测到完整的区域配置，请先完成区域选定</b></font>");
    return;
  }
  ...
  if (!timer->isActive()) {
    ...
    // 日志记录
    logTextEdit->append(
        QString("[模式1] 启动成功！监测间隔：%1秒").arg(intervalSeconds));
    logTextEdit->append(
        "[模式1] 开始监测检测区域变化，变化时将展示卡片区域截图");
  }
}
...
void MainWindow::newOCR() {
  ...
  if (!screen) {
    logTextEdit->append(
        "<font color='orange'><b>[模式1] 无法获取屏幕！</b></font>");
    return;
  }
  ...
  if (pixmap.isNull()) {
    logTextEdit->append(
        "<font color='orange'><b>[模式1] 截图失败！</b></font>");
    return;
  }
  ...
  if (!isMonitoringActive) {
    ...
    if (elapsedSeconds >= intervalSeconds) {
      ...
      m_croppedLastFrame = cropImageByPercent(
          currentFrame, detectStartX, detectWidth, detectStartY, detectHeight);
      ...
      if (!m_croppedLastFrame.empty()) {
        ...
        logTextEdit->append(
            QString("[模式1] 首次基准OCR完成，初始文本长度：%1字符")
                .arg(m_lastOcrText.length()));
      }
  ...
  try {
    ...
    if (croppedFrame.empty()) {
      logTextEdit->append(
          "<font color='orange'><b>[模式1] 红框检测区域裁剪失败！</b></font>");
      return;
    }
    ...
    if (changeRatio > MIN_CHANGE_RATIO) {
      // 红框区域变化，触发OCR对比逻辑
      logTextEdit->append(QString("<font color='red'><b>[模式1] "
                                  "红框区域发现变化！变化比例: %1%</b></font>")
                              .arg(changeRatio * 100, 0, 'f', 2));
      ...
      logTextEdit->append(
          QString("[%1] [模式1] 变化前OCR文本：")
              .arg(QDateTime::currentDateTime().toString("HH:mm:ss")));
      logTextEdit->append(lastOcrText.isEmpty() ? QStringLiteral("(空)")
                                                : lastOcrText);
      logTextEdit->append(
          QString("[%1] [模式1] 变化后OCR文本：")
              .arg(QDateTime::currentDateTime().toString("HH:mm:ss")));
      logTextEdit->append(currentOcrText.isEmpty() ? QStringLiteral("(空)")
                                                   : currentOcrText);

      // 7.
      // 提取新增文字,下面这个进行一个AI接入，让其自动判断是否有灾害消息，有的话就进行一个格式整理然后插入
      ...
      if (!newText.isEmpty()) {
        logTextEdit->append(
            QString("<font color='red'><b>[模式1] "
                    "检测到新增文字（新微信消息）：</b></font>"));
        ...
        if (!trimmedType.isEmpty()) {
          ...
          if (compactNewText.contains(compactType, Qt::CaseInsensitive)) {
            logTextEdit->append(QString("<font color='red'><b>[报警触发] "
                                        "新增文字命中监测类型：%1</b></font>")
                                    .arg(type));
            m_imageProcessor.StartAlert();
          }
      ...
      cv::rectangle(displayOriginal, cv::Point(redX, redY),
                    cv::Point(redX + redW, redY + redH), cv::Scalar(0, 0, 255),
                    2);
      ...
      QImage img2((const uchar *)displayRedFrame.data, displayRedFrame.cols,
                  displayRedFrame.rows, displayRedFrame.step,
                  QImage::Format_RGB888);
      croppedLabel->setPixmap(QPixmap::fromImage(img2).scaled(
          croppedLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
      croppedLabel->setText(""); // 清空提示

      logTextEdit->append(
          QString("[%1] [模式1] 红框区域变化 → 展示红框区域截图")
              .arg(QDateTime::currentDateTime().toString("HH:mm:ss")));
    ...
    std::cerr << "❌ OpenCV Exception: " << e.what() << std::endl;
    logTextEdit->append(
        QString("<font color='red'><b>[模式1] 异常：%1</b></font>")
            .arg(e.what()));
  } catch (...) {
    std::cerr << "❌ Unknown Exception!" << std::endl;
...
// 辅助函数：提取变化后独有的文字（新消息）
QString MainWindow::extractNewText(const QString &oldText,
