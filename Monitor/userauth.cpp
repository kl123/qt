...
static bool usersHasColumn(QSqlDatabase db, const QString& columnName, bool* outHasColumn, QString* errorMessage)
{
    if (outHasColumn) {
        *outHasColumn = false;
    }
...
static bool ensureUnitSchema(QSqlDatabase db, QString* errorMessage)
{
    ...
    if (!usersHasColumn(db, "dispatcher_user_id", &hasDispatcherUserId, errorMessage)) {
        return false;
    }
    ...
    if (!query.exec("CREATE INDEX IF NOT EXISTS idx_units_name ON units(name);")) {
        if (errorMessage) {
            *errorMessage = query.lastError().text();
        }
        return false;
...
static QString hashPasswordForStorage(const QString& password)
{
    ...
    const QByteArray digest = QCryptographicHash::hash(salt + pwd, QCryptographicHash::Sha256);
    return QString::fromLatin1(salt.toHex()) + "$" + QString::fromLatin1(digest.toHex());
}
...
static bool verifyPassword(const QString& stored, const QString& password)
{
    ...
    const QByteArray actual = QCryptographicHash::hash(salt + password.toUtf8(), QCryptographicHash::Sha256);
    ...
    QString* errorMessage)
{
    ...
    if (!usersHasColumn(db, "dispatcher_user_id", &hasDispatcherColumn, errorMessage)) {
        return false;
    }
    ...
    if (hasUnitIdColumn && hasDispatcherColumn) {
        query.prepare(
            "INSERT INTO users(username, password, phone, role, unit_id, dispatcher_user_id, unit) "
            "VALUES(?, ?, ?, ?, ?, ?, ?);");
        ...
        query.prepare(
            "INSERT INTO users(username, password, phone, role, unit) "
            "VALUES(?, ?, ?, ?, ?);");
    }
    ...
    query.addBindValue(phone.trimmed().isEmpty() ? QVariant(QVariant::String) : phone.trimmed());
    query.addBindValue(role.trimmed());

    if (hasUnitIdColumn && hasDispatcherColumn) {
        query.addBindValue(unitId > 0 ? QVariant(unitId) : QVariant(QVariant::LongLong));
        query.addBindValue(dispatcherUserId > 0 ? QVariant(dispatcherUserId) : QVariant(QVariant::LongLong));
        query.addBindValue(unitName.trimmed().isEmpty() ? QVariant(QVariant::String) : unitName.trimmed());
    } else if (hasUnitIdColumn) {
        query.addBindValue(unitId > 0 ? QVariant(unitId) : QVariant(QVariant::LongLong));
        query.addBindValue(unitName.trimmed().isEmpty() ? QVariant(QVariant::String) : unitName.trimmed());
    } else {
...
bool UserAuth::searchUnits(const QString& keyword, int limit, QList<UnitInfo>* outUnits, QString* errorMessage)
{
    ...
    QSqlQuery query(db);
    query.prepare("SELECT id, name FROM units WHERE name LIKE ? ORDER BY name LIMIT ?;");
...
bool UserAuth::searchDispatchersByUnit(
    qint64 unitId,
    ...
    QString* errorMessage)
{
    ...
    if (isHandlerRole(role)) {
        if (errorMessage) {
            *errorMessage = "现场处置员注册需要绑定指挥调度员，请使用 registerHandlerWithDispatcherId";
        }
        return false;
    }
    ...
    QString* errorMessage)
{
    ...
    QSqlQuery lookup(db);
    lookup.prepare(
        "SELECT u.unit_id, COALESCE(units.name, u.unit), u.role "
        "FROM users u "
        "LEFT JOIN units ON units.id = u.unit_id "
        "WHERE u.id = ? "
        "LIMIT 1;");
    ...
    QString* errorMessage)
{
    ...
    QSqlQuery query(db);
    query.prepare(
        "SELECT u.id, u.username, u.password, u.phone, u.role, COALESCE(units.name, u.unit), u.dispatcher_user_id "
        "FROM users u "
        "LEFT JOIN units ON units.id = u.unit_id "
        "WHERE u.username = ? "
        "LIMIT 1;");
